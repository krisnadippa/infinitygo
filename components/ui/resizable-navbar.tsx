  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        visible && "bg-white/90 shadow-2xl border border-slate-200/50 backdrop-blur-md",
        className,
      )}
    >
      {children}
      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className={cn(
              "fixed inset-y-0 right-0 z-[101] flex w-4/5 max-w-sm flex-col justify-start bg-white p-6 shadow-2xl dark:bg-neutral-950 h-screen overflow-y-auto border-l border-slate-100",
              className,
            )}
          >
            ...
            <div className="flex items-center justify-between pb-4 border-b border-slate-100 mb-2">
              ...
              <button onClick={onClose} className="p-2 rounded-full hover:bg-slate-100 text-slate-800 transition-colors">
                <IconX size={20} />
              </button>
            </div>
            {children}
...
export const NavbarButton = ({
  ...
  const variantStyles = {
    ...
    secondary: "bg-transparent text-white border border-white/20",
    dark: "bg-black text-white shadow-xl",
    gradient:
      "bg-gradient-to-b from-[#40B5AD] to-[#2d9d96] text-white shadow-lg",
    glass:
      "bg-white/40 text-slate-900 border border-white/60 shadow-sm hover:bg-white/60 hover:shadow transition-all",
  };
