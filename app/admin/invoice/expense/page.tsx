              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-[11.5px] text-slate-400">{inv.customerPhone} &bull; {inv.invoiceDate}</p>
                  </div>
                </div>
...
export default function ExpensePage() {
  ...
  const [expensesByInvoice, setExpensesByInvoice] = useState<Record<string, Expense[]>>(
    Object.fromEntries(dummyInvoices.map((inv) => [inv.id, [...inv.expenses]]))
  );
  ...
  const expenses = selectedInvoice ? (expensesByInvoice[selectedInvoice.id] || []) : [];
  const totalExpense = expenses.reduce((s, e) => s + e.amount, 0);
  const revenue = selectedInvoice?.grandTotal ?? 0;
  ...
  const handleSave = () => {
    if (!form.name || !form.amount || !selectedInvoice) return;
    if (editingExpense) {
      setExpensesByInvoice((prev) => ({
        ...prev,
        [selectedInvoice.id]: prev[selectedInvoice.id].map((e) =>
          e.id === editingExpense.id ? { ...e, ...form } as Expense : e
        ),
      }));
    } else {
      ...
      setExpensesByInvoice((prev) => ({
        ...prev,
        [selectedInvoice.id]: [...(prev[selectedInvoice.id] || []), newExpense],
      }));
    }
    setModalOpen(false);
  };
  ...
  const handleDelete = (id: string) => {
    ...
    setExpensesByInvoice((prev) => ({
      ...prev,
      [selectedInvoice.id]: prev[selectedInvoice.id].filter((e) => e.id !== id),
    }));
  };
    ...
    <div className="space-y-6">
            ...
            <div className="flex items-start justify-between mb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[12px] text-slate-400">{selectedInvoice.customerPhone} &bull; {selectedInvoice.invoiceDate}</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {expenses.map((exp) => (
                      <tr key={exp.id} className="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td className="px-5 py-3.5 text-[13px] text-slate-600">{exp.date}</td>
                        <td className="px-5 py-3.5">
                          <div className="flex items-center gap-2">
                            ...
                            <button onClick={() => handleDelete(exp.id)} className="p-1.5 rounded-lg hover:bg-red-50 text-slate-400 hover:text-red-500 transition-colors"><Trash2 size={14} /></button>
                          </div>
                        </td>
                      </tr>
                    ))}
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editingExpense ? "Edit Expense" : "Tambah Expense"}>
        <div className="space-y-4">
          ...
          <Input label="Tanggal" type="date" value={form.date || ""} onChange={(e) => setForm((p) => ({ ...p, date: e.target.value }))} id="expense-date" />
